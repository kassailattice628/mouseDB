...
df['generation'] = '0'#generation 列を作っとく
######
#index のみとり出し
df_sub_ind = pd.DataFrame({'mouse_id':df.index[:]})
######
#その他
sub = ['id', 'sex', 'status','line', 'gene', 'birth_date', 'death_date', 'father', 'mother', 'generation', 'user']

df_sub = df[sub].copy()
df_sub.columns = ['old_id', 'sex', 'status','line', 'genotype', 'birth_date', 'retire_date', 'father_id', 'mother_id', 'generation', 'user']
...
with sqlite3.connect(db) as conn:
    psql.to_sql(df_concat, 'individual', conn, index=False, if_exists='append')#'append', 'replace'
...
sql = "UPDATE individual SET generation = null"
c.execute(sql)
